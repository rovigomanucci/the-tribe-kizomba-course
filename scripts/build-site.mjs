...
function renderTimeline(lesson){return lesson.sequence.map((block,index)=>{const references=(videos[lesson.id]||[]).filter(video=>video.block===index);return '<details class="timeline-card" '+(index===0?'open':'')+'><summary class="timeline-summary"><span class="time">'+esc(block.time)+'</span><span><h3>'+esc(block.title)+'</h3><p>'+esc(block.task?.[0]||block.purpose)+'</p></span><span class="chevron">⌄</span></summary><div class="timeline-body"><div class="timeline-grid"><div class="mini"><span class="eyebrow">Student task</span>'+list(block.task)+'</div><div class="mini"><span class="eyebrow">Success signal</span><p>'+esc(block.success)+'</p></div></div>'+references.map(videoEmbed).join('')+'<button class="notes-button" data-note="'+index+'">View teaching notes →</button></div></details>'}).join('');}
